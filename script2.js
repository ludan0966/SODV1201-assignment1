...
const staffInfo = document.querySelector('#staffinformation');
let textContent = '';

function displayStaffInformation() {
  for (let i = 0; i < dataSet.length; i++) {
    for (let j = 0; j < 6; j++) {
      textContent += `${dataSet[i][j]} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`;
    }
    textContent += `<br>`;
  }
  staffInfo.innerHTML = textContent;
}
...
sortBySalarybtn.addEventListener('click', function () {
  for (let i = 0; i < dataSet.length - 1; i++) {
    for (let j = 0; j < dataSet.length - i - 1; j++) {
      const salary1 = Number(dataSet[j][5].replace(/[^0-9.-]+/g, ""));
      const salary2 = Number(dataSet[j + 1][5].replace(/[^0-9.-]+/g, ""));
